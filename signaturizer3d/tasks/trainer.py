...
class Trainer(object):
    def __init__(self, save_path=None, **params):
        ...
        if self.task != "repr" and self.task != "inference":
    ...
    def _init_trainer(self, **params):
        ...
        self.scaler = (
            ...
            if self.device.type == "cuda" and self.amp == True
            else None
        )
    ...
    def fit_predict(
        ...
        for epoch in range(self.max_epochs):
            ...
            message = (
                "Epoch [{}/{}] train_loss: {:.4f}, val_loss: {:.4f}, val_{}: {:.4f}, lr: {:.6f}, "
                "{:.1f}s".format(
                    epoch + 1,
                    self.max_epochs,
                    total_trn_loss,
    ...
    def _early_stop_choice(
        ...
        if not isinstance(self.metrics_str, str) or self.metrics_str in [
            ...
            (
                ...
                wait,
                max_score,
            ) = self.metrics._early_stop_choice(
                wait,
                min_loss,
                metric_score,
                max_score,
    ...
    def _judge_early_stop_loss(
        self, wait, loss, min_loss, model, dump_dir, fold, patience, epoch
    ):
        is_early_stop = False
        if loss <= min_loss:
            min_loss = loss
    ...
    def inference_direct(self, model, dataset):
        model = model.to(self.device)
        dataloader = NNDataLoader(
            dataset=dataset,
            batch_size=self.batch_size,
        ...
        for i, batch in enumerate(dataloader):
            net_input, _ = self.decorate_batch(batch)
            with torch.no_grad():
                # raw_outputs = model(**net_input)
                raw_outputs = model(**net_input, features_only=True)
                ...
                # raw_outputs_list.append(raw_outputs.cpu().numpy())
